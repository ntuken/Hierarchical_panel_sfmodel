...
function fit(sfdat::DataFrame) #, D1::Dict = _dicM, D2::Dict = _dicINI, D3::Dict = _dicOPT)
    ...
    # num: (nofobs, nofx, nofT, nofpara); number of variables in each equation
    ...
    β0     = xtvar \ yvar;         # OLS estiamte, uses a pivoted QR factorization;
    βₓ₀    = β0[1:num[:nofx]]       # retrieve OLS estimate for x-variables
    βₜ₀     = β0[num[:nofx] + 1 : end]   # retireve OLS estimate for time fixed effect
    ...
    if (:all_init in keys(_dicINI))
        ...
        b_ini  = get(_dicINI, :frontier, βₓ₀)
        t_ini  = get(_dicINI, :timevar, βₜ₀)
        ...
        init_vec = vcat(b_ini, t_ini, log_σ²ᵤ₀_ini, log_σ²ᵤ_star_ini, log_σ²₍₀_ini, log_σ²₍_star_ini, log_σ²w⁰_ini, log_σ²w_star_ini)  
    ...
    if _dicOPT[:silent] == false
        ...
        print("OLS (frontier & time only) log-likelihood: "); printstyled(round(ll_ols; digits=5); color=:yellow); println("")
